...
{
	vuint16_t CAM_READ1_STORE[NUM_OF_PIXEL] = {};
	vuint16_t CAM_READ2_STORE[NUM_OF_PIXEL] = {};
	...
	if ( CAM_RAN_FLAG )
	{
		...
		for ( i = 0; i < NUM_OF_PIXEL; i++ ) //0-127 >>> 1-128
		{
			...
			//print for test
			itoa(CAM_READ1_STORE[i], camOutTest);
			UART_print(camOutTest); UART_print(", ");
			...
			{	
				...
				else//if ( flag_disconti ){//불연속이다. 처음에는 연속이므로 저 밑으로 감
				{
					...
					{//연속 , 어두운지 확인. 
						...
						flag_disconti = 0;//연속
						index_DropEnd = i; 
						slope_dark = CAM_READ1_STORE[index_DropStart] - CAM_READ1_STORE[index_DropEnd];
	...
	/*####################### find location of line in pixel in camera 2 ###########################################*/
		...
		{//0-127 >>> 1-128
			...
			//print for test
			itoa(CAM_READ2_STORE[i], camOutTest);
			UART_print(camOutTest); UART_print(", ");
			
			
			if ( i > START_PIXEL - 2 && i < END_PIXEL + 1)
			{
				if ( minPhoto > CAM_READ2[i] ) minPhoto = CAM_READ2[i];
				if ( maxPhoto < CAM_READ2[i] ) maxPhoto = CAM_READ2[i];
			}
		}
			...
			{	
				...
				else//if ( flag_disconti ){//불연속이다. 처음에는 연속이므로 저 밑으로 감
				{
					...
					if ( abs(slope) > (maxdiff/EDGE_RATIO) ) {}//*1.6667 = 1/6, 다시 불연속확인, 불연속이면 씹음(필터링)
						...
						flag_disconti = 0;//연속
						index_DropEnd = i; 
						slope_dark = CAM_READ2_STORE[index_DropStart] - CAM_READ2_STORE[index_DropEnd];
						//CAM_READ1_STORE[index_DropStart] 아직 밝은값, 이 다음 픽셀부터 어두움. 
						//CAM_READ1_STORE[index_DropEnd]가 DropStart보다 maxdiff/2 이상 어두운가.
