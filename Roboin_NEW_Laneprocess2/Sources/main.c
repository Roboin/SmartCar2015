...
void Servo_Control( int16_t* current_servo_angle_pt  ){/*cam1LanePositionReturn(), cam1LanePositionReturn(), flag*/
	...
	int16_t Rdatum = 84;//71;//43;//30; 정상 상태 왼쪽 선 위치
	int16_t Ldatum = 38;//91;//74;//100; 정상 상태 오른쪽 선 위치
	...
	{//정상경우
		...
		cont_val_P_Servo = kpServo*error_center;
		cont_val_I_Servo += kIServo*error_center;
		...
		//정비례 모드
		ServoAngle += error_center*kpServo/500;//(Speed_Proposal_inMain * Speed_Proposal_inMain )*error_center/kpServo;//*kpServo;
		ServoPulse = (cont_val_P_Servo + cont_val_I_Servo)/10;//error_center*kpServo/10;
	...
	*current_servo_angle_pt = ServoPulse;
