...
//####### Pin Guide ##########
int16_t CAM_DATA_PIN1 = 4;//4=pd0//14=GPIO_PD10;//PD10;
...
void CAM_RUN(void)
{
	//CAM_SI();//For trigger Pulse
	if(CAM_SI_FLAG==1){
		GPIO_Set(CAM_SI_PIN,1);
		t0_CAM_SI = CAM_MICRO_SEC;//BTMR_GetRuntime_Micros();//
		CAM_SI_FLAG = 2;
	}
	if(CAM_SI_FLAG==2){
...
{
	int32_t delay_val = 0;
	delay_long = (delay_long << (4+NGV_DMU_SYSCLK))/5; 
