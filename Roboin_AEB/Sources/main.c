#include "stdlib.h"
#include "stdio.h"

#include "ngv_dmu.h"
#include "rappid_ref.h"
#include "rappid_utils.h"
#include "sys_init.h"
#include "gpio.h"
#include "demoled.h"
#include "demoswitch.h"
#include "basetimer.h"
#include "serialdebug.h"
#include "myuart.h"
#include "youruart.h"
#include "lcd.h"

#include "freerunadc.h"
#include "freerunusonic.h"
#include "rotaryencoder.h"
#include "motorcontrol.h"
#include "camera.h"

#include "adc_drv.h"

void InitPeripherals(void);
void DoMainLoop(void);

void Speed_Control(int16_t proposal_speed);
void itoa(int num, char *pStr);

void AEB();

void InitPeripherals(void)
{
	sys_init_fnc();
	DLED_Init();
	DSW_Init();
	BTMR_Init();
	SDBG_Init();
	MUART_Init();
	YUART_Init();
	//CLCD_Init();
	FADC_Init(0xfff3);
	FUSS_Init(FUSS_CH_ALL);
	RENC_Init();
	MCTL_Init();
	CAM_Init();
	//LCD test
	LCD_initialize();
	
	EnableExternalInterrupts();
}

//##########DO MAIN LOOP################//
int8_t flag = 1;
int16_t i_serial=0;
uint16_t cam_th= 200;
uint16_t dipsw = 0;
int16_t blinking_LED = DLED_LED1;

int16_t time_gap = 250;

int16_t sAngle = 0;

int32_t enc1_current=0, enc1_old=0, enc2_current=0, enc2_old=0;
int32_t enc1_rate=0, enc2_rate=0;


			// 0123456789012345";
char strA[] = "Mod:SCAN TH:    ";
char strB[] = "MAX:    MIN:    ";
char LCD_ST_BUFF[6];
char LCD_MAX_BUFF[6];
char LCD_MIN_BUFF[6];
char LCD_TH_BUFF[6];

int runMode = 1;
int lcd_num = 0;
uint32_t FUSS_VALUE = 0;
uint8_t IR_ADC_NUM = 6;
int16_t DC_Speed = 500;
int16_t Proposal_Speed = 5;

uint8_t flag_US = 0;
uint8_t flag_IR = 0;
uint8_t flag_Slope = 0;
uint8_t flag_Wall = 0;

void DoMainLoop(void)
{
	//COUNT FOR TIMER
	static uint32_t t_current, t_old=(uint32_t)-1, t_test,cnt=0;
	//CAM RUN mode 1 or 2
	if(dipsw & 0b0001){//sw1 //0x1
		CAM_RUN_MODE_SETUP(2);//up
		DLED_Set(DLED_LED1,0);
		blinking_LED= DLED_LED2;
		CAM_RUN2();
	}
	else{
		CAM_RUN_MODE_SETUP(1);//down
		DLED_Set(DLED_LED2,0);
		blinking_LED= DLED_LED1;
	}
	
	//SERIAL COMMUNICATE
	if (SDBG_IsEvents())
		SDBG_ExecuteCallback();
	
	if (MUART_IsEvents())
		MUART_ExecuteCallback();
	
	//servo test in main
	sAngle = 0;//(FADC_Get(1)-870)*2*37/156-37;
	MCTL_Servo(sAngle);
			
	//DC test
	Speed_Control(Proposal_Speed);
	MCTL_DC_R(DC_Speed);
	MCTL_DC_L(DC_Speed);
	
	//##################  AEB  #####################
	FUSS_VALUE = FUSS_GetMm();
	
	if( FUSS_VALUE < 300){// Near 60cm
		//GPIO_Set(DLED_LED3,0);
		Proposal_Speed = 0;
		//Speed_Control(50);//Deceleration
		flag_US = 1;
		AEB();
	}
	else{
		flag_IR = 0;
		flag_US = 0;
		flag_Wall = 0;
		flag_Slope = 0;
		Proposal_Speed = 5;
		//GPIO_Set(DLED_LED3,1);
	}
	GPIO_Set(DLED_LED3,!flag_Slope);
	GPIO_Set(DLED_LED4,!flag_Wall);
	
	
	//TIMER FOR EACH N ms	
	time_gap = 500;
	t_current = BTMR_GetRuntime()/time_gap;
	if (t_current != t_old) {
		t_old = t_current;
		DLED_Toggle(blinking_LED);
		
		t_test = CAM_MICRO_SEC_RETURN();
		//SDBG_Printf("\n%u: %d", cnt++, t_test);
		
		//AEB Sensor Value
		SDBG_Printf("\nUS : %8u  IR : %4d", FUSS_VALUE,FADC_Get(IR_ADC_NUM));
		
		//LCD Test
		/*LCD_ON();//x, y,*string
		LCD_string(0, 0, strA);
		LCD_string(0, 1, strB);
		//itoa( (FADC_Get(1)-800)*5 ,LCD_TH_BUFF);
		itoa( cam_th ,LCD_TH_BUFF);
		itoa(CAM_MAX(),LCD_MAX_BUFF);
		itoa(CAM_MIN(),LCD_MIN_BUFF);
		LCD_string(12, 0, LCD_TH_BUFF);
		LCD_string(4, 1, LCD_MAX_BUFF);
		LCD_string(12, 1, LCD_MIN_BUFF);*/
		
		//DIPSW - 1-Camera Run / 2-cam_th / 3-.! or %3d / 4-cam_pulsewidth(exposuretime)
		dipsw = DSW_GetDipAll();
		
		//Threshold
		if(dipsw & 0b0010)//sw2 //0x2
			cam_th = (uint16_t)(FADC_Get(1)-800)*5;
		else
			cam_th = CAM_AVG();
		//camera exposure time
		if(dipsw & 0b1000)//sw4//0x8
			CAM_PulseWidth(100);//up
		else//down
			CAM_PulseWidth(50);
		
		//ENCODER TEST
		//*/
		enc1_current = RENC_Get1();
		enc2_current = RENC_Get2();
		enc1_rate = enc1_current - enc1_old;
		enc2_rate = enc2_current - enc2_old;
		enc1_old = enc1_current;
		enc2_old = enc2_current;
		//SDBG_Printf("\n%u: Speed : %4d Rval : %4d %4d    RATE : %4d %4d", cnt++, DC_Speed,RENC_Get1(),RENC_Get2(), enc1_rate, enc2_rate);
		
		//Speed_Control(3);//3cm/s
		//DC_Speed = (FADC_Get(1)-800)*10;//(A2D_GetChResult_10bit(1)-800)*10;
		//MCTL_DC_R(DC_Speed);
		//MCTL_DC_L(DC_Speed);
		//SDBG_Printf("\n%u: Speed : %4d Rval : %4d RATE : %4d", cnt++, DC_Speed,RENC_Get1(), enc1_rate);
		//SDBG_Printf("\n%u: US : %8u  IR : %4d", cnt++, FUSS_VALUE,FADC_Get(IR_ADC_NUM));//adc6 = pd2 = IR
		/**/
		
		//Servo test
		//MCTL_Servo(sAngle);
		//sAngle +=5;
		if(sAngle > MCTL_SWHEEL_MAX)
		{
			sAngle = -MCTL_SWHEEL_MAX;
		}
		//SDBG_Printf("\nServo Angle : %d",sAngle);
		
		//if(dipsw & 0x8){//sw4, SDBG
			if(flag==0){
				//flag = 1;
			}
		//}
	}
	
	if(flag){
		SDBG_Printf("\n %4d TH:%3d,%3d,%3d\t",t_current,cam_th,CAM_MAX(),CAM_MIN());
		for(i_serial = 0; i_serial < NUM_OF_PIXEL; i_serial+=2){//i_serial++){
			if(dipsw & 0b0100){//up//0x04
				if(CAM_DATA1(i_serial) > cam_th){
					SDBG_Printf(".");
				}
				else{ //if(CAM_READ(i_serial) , cam_th)
					SDBG_Printf("!");
				}
			}
			else{//down
				SDBG_Printf("%3d ",CAM_DATA1(i_serial));
			}
		}
		SDBG_Printf("||");
		for(i_serial = 0; i_serial < NUM_OF_PIXEL; i_serial+=2){//i_serial++){
			if(dipsw & 0b0100){//up//0x04
				if(CAM_DATA2(i_serial) > cam_th){
					SDBG_Printf(".");
				}
				else{ //if(CAM_READ(i_serial) , cam_th)
					SDBG_Printf("!");
				}
			}
			else{//down
				SDBG_Printf("%3d ",CAM_DATA2(i_serial));
			}
		}
		flag = 0;
	}
}

void Speed_Control(int16_t proposal_speed){
	int16_t pulse_per_cm = 11;
	int32_t proposal_enc_rate = pulse_per_cm*proposal_speed*time_gap/1000;
	int16_t kp = 10;
	if(enc1_rate > proposal_enc_rate){
		DC_Speed -= kp;
	}
	if(enc1_rate < proposal_enc_rate){
		DC_Speed += kp;
	}
}


void AEB(){				// 10   20   30   40   50   60
	uint16_t IR_POINT[6] ={500, 500, 420, 330, 250, 210};//ADC value data for 10-50cm
	int16_t ReverseSpeed = -2047;
	int16_t AEB_Distance = 30;
	if(flag_US){
		if(A2D_GetSingleCh_10bit(IR_ADC_NUM) > IR_POINT[AEB_Distance/10-1] ){//a2d(6) = PD1, NEED CONFIRM!
			//Stop the car.
			MCTL_DC_R(ReverseSpeed);
			MCTL_DC_L(ReverseSpeed);
			BTMR_DelayMs(1);
			MCTL_BrakeEngine();
			//GPIO_Set(DLED_LED4,0);
			flag_IR = 1;
			flag_Wall = 1;
		}
		else{//°æ»ç·Î, Re Acceleration
			Speed_Control(25);
			flag_IR = 0;
			//GPIO_Set(DLED_LED4,1);
			flag_Slope = 1;
		}
	}
	//IR detected
	if(flag_IR){
		MCTL_BrakeEngine();
		GPIO_Set(GPIO_PA6,0);
	}
	
}



int main(void)
{
	InitPeripherals();     
	//SDBG_RegisterSmallCmd('s', Callback_s);
	//SDBG_RegisterSmallCmd('e', Callback_e);
    
	//MUART_Printf("\nCAMERA Start!\n");
	SDBG_Printf("\nAEB Start!\n");
	
	while (1)
		DoMainLoop();   
	return 0;
}

//############## int to char ###################
void itoa(int num, char *pStr){
    int radix = 10;
    int deg = 1;       
    int cnt = 0;       
    int i;

    if(pStr == NULL) return ;

    if(num < 0){
        *pStr = '-';
        num *= -1;
        pStr++;
    }

    while(1){
        if( (num / deg) > 0)
            cnt++;
        else
            break;

        deg *= radix;
    }
    deg /= radix;

    for(i = 0; i < cnt; i++, pStr++){
        *pStr    = (char)(num / deg) + '0';
        num        -= (num / deg) * deg;
        deg        /= radix;
    }
    *pStr = '\0';
}
